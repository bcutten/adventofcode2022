...
total_size = 0  
#list to store names and sizes of all directories that will free up enough space
to_delete = []
...
def find_sum(directory):
    ...
    if size <= 100000:
        ...
        total_size += size
    elif size >= 3441553: #candidate for deletion
        to_delete.append([directory["name"], size])
    return size

#find the smallest item in the list
def find_smallest(to_delete):
    smallest = 100000000
    #check each directory
    for directory in to_delete:
        #is is smaller than the current smallest?
        if(int(directory[1]) < smallest):
            #becomes the new smallest
...
with open('input7.txt') as f:
    ...
    #now the entire file structure is built
    
    #Figure out how much we need to delete
    current_size = (find_sum(root))
    free_space = 70000000 - current_size
    needed = 30000000 - free_space
    print(needed)
    #hardcoded this value into the above method
    #now find the best directory tp delete
    find_smallest(to_delete)